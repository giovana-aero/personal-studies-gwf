...
% This code is an adaptation of the flat plate problem, consisting mainly
% of modifications to the boundary conditions:
% - The upper boundary is replaced by another wall
% - Leading edges are moved back

% Tests:
% H = L/3.00 - ok
% H = L/4.00 - ok (about 500 iterations)
% H = L/5.00 - ok (825 iterations)
% H = L/6.00 - ok (1028 iterations)
% H = L/6.70 - ok (2280 iterations)
% H = L/6.71 - ok (2447 iterations)
% H = L/6.72 - ok (2731 iterations)
% H = L/6.73 - ok (4000 iterations (no convergence)) - good result, mach number a little too high probably
% H = L/6.74 - broken (5691 iterations)
% H = L/6.80 - broken (3460 iterations)
% H = L/6.90 - broken (2527 iterations)
% H = L/7.00 - broken (2148 iterations)
% (same initial conditions as the flat plate, 20x20 mesh, mach 4)

% Tests:
% H = L/1.00 - ok (639 iterations)
% H = L/3.00 - ok (928 iterations)
% H = L/4.00 - broken (81 iterations)
% H = L/5.00 - broken (1 iteration)
% (same initial conditions as the flat plate, 50x50 mesh, mach 4)

% Tests:
% H = L*2.00 - broken (1080)
% H = L/1.00 - broken (935 iterations)
% H = L/2.00 - ok (4000 iterations (no convergence)) - shock wave "walks" ahead of the leading edges
% H = L/3.00 - ok (10000 iterations (no convergence)) - same as above
% (same initial conditions as the flat plate, 50x50 mesh, mach 1.5)
...
L = 0.00001; % Length of computational domain (inlet to outlet) [m]
%Re = u1*L*rho1/mu1;
%H = 5*(5*L/sqrt(Re)); % Height of the computational plane [m]
H = L*1;
Tw = T1; % Temperature of the plate at the surface [K]
D = 1/8; % Distance from the inlet to the leading edges (in terms of L)
...
step_num = 20; % Number of steps to update the mach number contour graph
...
delta_x = L/(NX-1);
delta_y = H/(NY-1);
LEx = floor(NX*D); % Leading edge location
...
% Load functions
c10_normal_shock_functions;

% Boundary conditions
[u_mat,v_mat,T_mat,p_mat,rho_mat,e_mat,mu_mat] = boundaries(u_mat,v_mat,T_mat,p_mat,rho_mat,e_mat,mu_mat,u1,v1,T1,p1,rho1,mu1,cv,R,Tw,NX,NY,LEx);
...
% Start step number (to update the mach number contour)
step = 1;
...
for loop = 1:iter
    ...
    [u_bar,v_bar,T_bar,p_bar,rho_bar,e_bar,mu_bar] = boundaries(u_bar,v_bar,T_bar,p_bar,rho_bar,e_bar,mu_bar,u1,v1,T1,p1,rho1,mu1,cv,R,Tw,NX,NY,LEx);
    ...
    if step == step_num
        plot_M_contours(u_mat,v_mat,a_mat)
        title(['Iteration ',num2str(loop)])
%        saveas(gcf,['figs/graph_iter',num2str(loop),'.png'])
        step = 1;
    else
        step = step + 1;
    end
