...
% As seções transversais da parte da frente e da seção de transição podem
% ser modeladas como ovos ou elipses. A cauda consiste apenas de elipses
...
% Pontos na seção transversal (vale para a geometria completa)
n_elipse = 50;

% Configuração das seções transversais
shape_v = repelem(2,1,n_sec); % Altera o formato do ovo/elipse
op_v = repelem(1,1,n_sec); % Altera a equação a ser utilizada
...
% Configuração do cosspace2 (isto afeta o formato da seção de transição)
CP_factor1 = 0; % lados
CP_factor2 = 0; % cima e baixo

% Configuração das seções transversais
shape_v2 = repelem(2,1,n_sec); % Altera o formato do ovo/elipse
op_v2 = repelem(1,1,n_sec); % Altera a equação a ser utilizada


%% Cálculos referentes à parte da frente ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

if fechar == 1 % Opção de fechamento do contorno
    num = 1;
else
    num = 2;
end
...
for i = 2:n_sec
     ...
     %raios da elipse
     yRadius = spl_gv_L_half(i)*size_f_L; % abscissa
     zRadius = delta_z(i)*size_f_CB;      % ordenada

     %geração da elipse baseando-se em um número n de pontos
%     z = linspace(-(zRadius/2+zCenter),zRadius/2+zCenter,n_elipse);
     z = linspace(-zRadius,zRadius,n_elipse/2);
     y = egg_formulas(z,yRadius*2,zRadius*2,shape_v(i),op_v(i));
     z = [z,flip(z(num:end-1))]+zCenter; y = [y,-flip(y(num:end-1))];

%     figure(1),clf,plot(y,z),grid on,axis equal,hold on,scatter(y,z)
...
%     elipsez(i).coo = [repelem(xx2(i),n_elipse+1) y' z'+spl_camber(i)*size_f_CB];
    ...
    if invert_yz == 1
        elipsez(i).coo = [repelem(xx2(i),n_elipse-num)',z'+spl_camber(i)*size_f_CB,y'];
    else
        elipsez(i).coo = [repelem(xx2(i),n_elipse-num)',y',z'+spl_camber(i)*size_f_CB];
    end
end
...
for i = 2:n_sec2
     ...
     %geração da elipse baseando-se em um número n de pontos
     z = linspace(-zRadius,zRadius,n_elipse/2);
     y = egg_formulas(z,yRadius*2,zRadius*2,shape_v2(i),op_v2(i));
     z = [z,flip(z(num:end-1))]+zCenter; y = [y,-flip(y(num:end-1))];
%     theta = linspace(0,360,n_elipse+1)+90;
%     theta = flip(theta);
...
%     elipsez2(i).coo = [repelem(x2(i),n_elipse+1),y',z'];
    if invert_yz == 1
        elipsez2(i).coo = [repelem(x2(i),n_elipse-num)',z',y'];
    else
        elipsez2(i).coo = [repelem(x2(i),n_elipse-num)',y',z'];
    end
...
for i = 2:n_sec3
 ...
 %     z(:,n_elipse+1)=[]; %removendo a última linha dessa matriz
     ...
     if invert_yz == 1
         elipsez3(i).coo = [repelem(x3(i),n_elipse+1)',z',y'];
     else
...
%    fprintf(arquivo,'%f %f %f\n', size_f_CB*coo_gv_CB_pl_mod');
    ...
    % É necesário que não haja caracteres especiais (acentos e afins)
    place = '[diretorio_aqui]';
