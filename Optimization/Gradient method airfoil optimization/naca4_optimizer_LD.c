...
int main(){
  ...
  for(int i=0;i<opt_iter;i++){
    gradient(&grad,airfoil_data,params,numPoints,delta_m);
    airfoil_data[0] += step*grad;
    opt_func(&LD,airfoil_data,params,numPoints);
    printf("Iteration %d -> f(%f,%f,%f) = %f | %f\n",
           i+1,airfoil_data[0],airfoil_data[1],airfoil_data[2],LD,grad);
    // getchar();
    history[i] = LD;
    grad_history[i] = grad;
    if(fabs(grad) < eps){
  ...
  putchar('\n');
  for(int i=0;i<opt_iter;i++){
    printf("%f ",grad_history[i]);
  }
  putchar('\n');

  results = fopen("naca4_results.txt","w");
  for(int i=0;i<opt_iter;i++){
    fprintf(results,"%.6E %.6E\n",history[i],grad_history[i]);
  }
  fclose(results);
...
void gradient(double *grad,double *airfoil_data,double *params,int numPoints,
              double delta_m){
  double airfoil1[3], airfoil2[3];
  double LD1, LD2;
  airfoil1[0] = airfoil_data[0] + delta_m;
  airfoil1[1] = airfoil_data[1];
  airfoil1[2] = airfoil_data[2];
  opt_func(&LD1,airfoil1,params,numPoints);
  airfoil2[0] = airfoil_data[0] - delta_m;
  airfoil2[1] = airfoil_data[1];
  airfoil2[2] = airfoil_data[2];
  opt_func(&LD2,airfoil2,params,numPoints);

  *grad = (LD1-LD2)/(2*delta_m);
}
