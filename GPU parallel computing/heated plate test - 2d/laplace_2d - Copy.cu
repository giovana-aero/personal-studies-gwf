...
void save_sol_old(int Nxs,int Nys,double *sol,double *sol_old,int k){
  ...
  int stride2 = blockDim.y*gridDim.y;
	printf("%d %d %d %d\n",index1,stride1,index2,stride2);
	// printf("%s\n",cudaGetErrorString(cudaGetLastError()));
	for(int j=index1;j>0&&j<Nys-1;j+=stride1){
		for(int i=index2;i>0&&i<Nxs-1;i+=stride2){
			// printf("%s\n",cudaGetErrorString(cudaGetLastError()));
			sol_old[k*(Nxs-2)*(Nys-2)+(Nys-2)*(j-1)+(i-1)] = sol[k*Nxs*Nys+Nys*j+i];
			// printf("%s\n",cudaGetErrorString(cudaGetLastError()));
			printf("(%f | %f)\n",sol[k*Nxs*Nys+Nys*j+i],sol_old[k*(Nxs-2)*(Nys-2)+(Nys-2)*(j-1)+(i-1)]);
			// printf("%s\n",cudaGetErrorString(cudaGetLastError()));
		}
	}
}
...
void parallel_solver(int Nxs,int Nys,double *sol,double *sol_old,double *res,
	...
	for(int loop=0;loop<iter;loop++){
		...
		iterate_once<<<1,256>>>(Nxs,Nys,sol,sol_old,res,beta);
